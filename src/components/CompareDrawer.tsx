      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-wine/70 backdrop-blur-md z-[80]"
          />
          ...
          <motion.div
            ...
            className={`fixed top-0 bottom-0 ${isRTL ? 'left-0' : 'right-0'} w-full sm:w-[440px] bg-white text-wine dark:bg-[#09090b] dark:text-white z-[90] shadow-2xl flex flex-col border-x border-wine/10 dark:border-white/10`}
          >
            {/* Drawer Header */}
            <div className="p-6 border-b border-wine/10 dark:border-white/10 flex items-center justify-between bg-zinc-50/80 dark:bg-wine/50 backdrop-blur-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xs luxury-tracking font-bold uppercase tracking-[0.2em] text-wine dark:text-white">
              ...
              <button
                onClick={onClose}
                className="p-2.5 text-zinc-400 hover:text-wine dark:hover:text-white hover:bg-wine/5 dark:hover:bg-white/10 rounded-full transition-colors cursor-pointer"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {comparedProducts.length === 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-12 px-4 space-y-5">
                  <div className="w-20 h-20 rounded-full bg-zinc-100 dark:bg-white/5 border border-wine/10 dark:border-white/10 flex items-center justify-center text-zinc-400 dark:text-zinc-500 shadow-inner">
                    <ArrowLeftRight size={32} strokeWidth={1.3} />
                  </div>
                  
                  <div className="max-w-[280px] space-y-2">
                    <h3 className="text-xs uppercase luxury-tracking font-bold tracking-[0.15em] text-wine dark:text-white">
                  ...
                  <button
                    ...
                    className="mt-2 px-6 py-3 rounded-xl bg-wine text-white dark:bg-white dark:text-wine text-[11px] uppercase tracking-wider font-bold hover:opacity-90 transition-opacity cursor-pointer shadow-sm"
                  >
                  ...
                  <AnimatePresence mode="popLayout">
                    {comparedProducts.map((product) => (
                      <motion.div
                        ...
                        className="flex gap-4 p-3 rounded-2xl border border-wine/10 dark:border-white/10 bg-zinc-50/80 dark:bg-white/5 relative group hover:border-amber-500/50 transition-all shadow-xs"
                      >
                        {/* Thumbnail */}
                        <div 
                          onClick={() => {
                        ...
                        >
                          ...
                          />
                          <div className="absolute inset-0 bg-wine/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                          ...
                          <div>
                            <div className="flex items-start justify-between gap-2">
                              <h3 
                                ...
                                className="text-xs uppercase font-bold text-wine dark:text-white truncate cursor-pointer hover:text-amber-500 transition-colors"
                              >
                            ...
                            <button
                              ...
                              className={`flex-1 py-1.5 px-3 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                                product.isSoldOut
                                  ...
                                  : 'bg-wine text-white dark:bg-white dark:text-wine hover:opacity-90'
                              }`}
                            >
                              <ShoppingBag size={12} />
            ...
            {comparedProducts.length > 0 && (
              <div className="p-6 border-t border-wine/10 dark:border-white/10 bg-zinc-50/80 dark:bg-wine/50 space-y-3">
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl bg-amber-400 text-wine hover:bg-amber-300 font-extrabold text-xs uppercase tracking-widest shadow-lg shadow-amber-400/20 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
                >
