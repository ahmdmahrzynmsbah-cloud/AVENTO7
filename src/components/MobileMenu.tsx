      ...
      {isOpen && (
        <>
          ...
          >
            ...
            <div className="mb-6 flex flex-col gap-3 pb-4 border-b border-black/10 dark:border-white/10">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-black/20 dark:border-white/20 transition-all group bg-black/5 dark:bg-white/5"
                >
                  {theme === 'dark' ? (
                    <>
                    ...
                    <>
                      <Moon size={13} className="text-zinc-900" />
                      <span className="text-[9px] luxury-tracking font-semibold text-zinc-900">{lang === 'ar' ? 'الوضع الليلي' : 'NIGHT MODE'}</span>
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="mb-8 flex flex-col w-full">
              {user ? (
                <div className="flex flex-col items-start gap-1 text-[10px] luxury-tracking uppercase text-zinc-500 dark:text-white/50">
                  ...
                  <div className="text-lg serif-display tracking-widest uppercase text-zinc-900 dark:text-white font-light normal-case mb-4">{user?.name || (lang === 'ar' ? 'عضو' : 'MEMBER')}</div>
                  ...
                  <div className="flex flex-col gap-3 mt-2 font-medium w-full text-[9px] tracking-[0.2em]">
                    ...
                    <button 
                      ...
                      className="text-left rtl:text-right w-full hover:text-black dark:hover:text-white transition-colors flex justify-between uppercase py-1"
                    >
                    ...
                    <button 
                      ...
                      className="text-left rtl:text-right text-zinc-500 hover:text-black dark:text-[#86868b] dark:hover:text-white transition-colors uppercase pt-2"
                    >
                ...
                <div className="flex flex-col gap-4 items-start w-full border-b border-black/10 dark:border-white/10 pb-8">
                  ...
                  <button 
                    ...
                    className="flex justify-between items-center text-left rtl:text-right w-full group py-3 text-zinc-900 hover:text-zinc-600 dark:text-white dark:hover:text-[#86868b] transition-colors"
                  >
            ...
            {/* Search Section */}
            <div className="mt-16 flex items-center gap-4 border-b border-black/20 dark:border-white/20 pb-4 group mb-12">
              <Search size={18} strokeWidth={1.5} className="text-zinc-400 dark:text-white/50 group-focus-within:text-black dark:group-focus-within:text-white transition-colors" />
              <input 
                type="text" 
                placeholder={lang === 'ar' ? 'بحث...' : 'SEARCH...'} 
                className="bg-transparent border-none outline-none text-[11px] luxury-tracking text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-white/30 w-full uppercase text-left rtl:text-right"
              />
            </div>
