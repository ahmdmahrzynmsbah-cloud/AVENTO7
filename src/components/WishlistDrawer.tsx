      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-full sm:w-[420px] bg-white text-wine dark:bg-[#080808] dark:text-white z-[90] shadow-2xl flex flex-col border-l border-wine/10 dark:border-white/10"
          >
            {/* Header */}
            <div className="p-6 border-b border-wine/10 dark:border-white/10 flex items-center justify-between">
              ...
              <button
                onClick={onClose}
                className="p-2 hover:bg-wine/5 dark:hover:bg-white/10 rounded-full transition-colors cursor-pointer"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {wishlistItems.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-16 space-y-4 text-zinc-400 dark:text-white/40">
                  <div className="w-16 h-16 rounded-full bg-zinc-100 dark:bg-white/5 flex items-center justify-center border border-wine/10 dark:border-white/10">
                ...
                wishlistItems.map((product, idx) => (
                  <div
                    key={`${product.id}-${idx}`}
                    className="flex gap-4 p-3 border border-wine/10 dark:border-white/10 bg-zinc-50/50 dark:bg-white/5 relative group"
                  >
                    <div className="w-20 h-24 bg-zinc-100 dark:bg-wine shrink-0 overflow-hidden relative">
                      <img
                        src={product.image}
                        alt={product.name}
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1 min-w-0">
                      <div>
                        <h3 className="text-xs uppercase font-bold text-wine dark:text-white truncate">
                        ...
                        <button
                          ...
                          className={`flex-1 py-2 font-bold uppercase text-[9px] luxury-tracking transition-colors flex items-center justify-center gap-1.5 ${
                            product.isSoldOut
                              ...
                              : 'bg-wine text-white dark:bg-white dark:text-wine hover:bg-zinc-800 dark:hover:bg-white/80 cursor-pointer'
                          }`}
                        >
                          <ShoppingBag size={12} />
            ...
            {/* Footer */}
            {wishlistItems.length > 0 && (
              <div className="p-6 border-t border-wine/10 dark:border-white/10 bg-zinc-50 dark:bg-[#050505]">
                <button
                  onClick={() => {
                    wishlistItems.forEach((p) => onAddToCart(p, p.sizes?.[0] || 'M'));
                    wishlistItems.forEach((p) => onRemoveFromWishlist(p.id));
                  }}
                  className="w-full py-4 bg-amber-500 text-wine font-bold uppercase text-[10px] luxury-tracking tracking-[0.2em] hover:bg-amber-400 transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-lg"
                >
                  <ShoppingBag size={16} />
